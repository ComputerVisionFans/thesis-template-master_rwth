...
\label{fig:cellnet}
\end{center}
\caption{ResNet18 [17] and shuffleNetv2 [25]  Our Net performance on Sezary SyndromeDataset. Our net  achieves higher classification perfor-mance (e.g. 95.638\% top-1 accuracy ) than ResNet 18 ,ShuffleNet V2  and GhostNet, while less weights andcomputational cost.}
\end{figure}
...
\label{sec:ipsum}
The demonstration of Pneumonia dataset. Author in  built the standard benchmark Pneumonia dataset. It contains nearly 2000 patients CT images, the training set contain two classes Normal 1341 images and Pneumonia 3875 images, the Val set contains 242 normal images and 398 pneumonia images
...
\label{sub:amet}To verify the effectiveness and generalization of the proposed Our*Net architecture, we conduct experiments on several benchmark visual datasets, including CIFAR-10. And we compare the performance with seveal so far best  representative state-of-the-art models.
\begin{table}[t]
\centering
\begin{tabular}{@{}llll@{}}
\toprule
...
\textbf{Squeeze-and-Excitation(SE Layer): scaling inhibiting vs intrinsic features map.} Squeeze-and-Excitation (SE)\cite{24} adaptively recalibrates channelwise feature responses by explicitly modeling inter-dependencies between channels. 
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \Ensure
     $pathlist$, $predictionlabellist$ and $targetlabellist$
  
    \If inputcheck buttion is checked 
      \State  read image from the globrec function and list image path into a sorted list:
      mypictures \gets globrec($Path$)
      \State  calculate the lenght of the image list:
      lenght of mypictures \gets len($mypictures$)
      \State  enable the QtWidgets to send a message box to give the user hins
      box  \gets QtWidgets.QMessageBox()
      \State  enable the Qtexteditor to read for the future output results:
      \set hins buttons = \True 
      \State  calculate the category possibility using the model trained $Cellnet$ and best weight:$pt$:where Precision ,$Recall
      
      $predictionlabellist$, $targetlabellist$,$Precision$,$Recall$ \gets Cellnet($Path$,$pt$)
